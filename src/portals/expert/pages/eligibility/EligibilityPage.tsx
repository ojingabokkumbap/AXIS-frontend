...
import { expertApi, type EligibilityRow } from '@expert/services/api';
import { AxiosError } from 'axios';
import {
  ELIG_REJECT_REASONS,
  encodeEligibilityRejectNote,
  type EligibilityRejectCode,
} from '@expert/constants/eligibilityRejectReasons';
...
async function fetchEligibilityDoc(registrationId: string) {
  const res = await expertApi.getEligibilityDoc(registrationId);
  if (!res.data.url) return null;
  return { url: res.data.url, fileName: res.data.fileName ?? 'eligibility-document' };
}

export default function EligibilityPage({ onPendingChanged }: { onPendingChanged?: () => void }) {
  ...
  const review = async (
    ...
    try {
      await expertApi.reviewEligibility(registrationId, decision, note);
      pushToast(
  ...
  return (
    <div>
      <PageHeader
        title="L1 응시자격 검토"
        subtitle="응시자가 등록 시 업로드한 AXIS-C L1 자격 증빙 서류가 자동으로 표시됩니다. 별도 배정 없이 검토·승인·반려할 수 있습니다."
      />
